...
\selfnote{make more technical}
\selfnote{make flowchart of processes in project}
...
We note that the number of transfers and the path link fields of some records do not correspond to the infrmation in their trip details. According to domain expert PhD. Tu Qiang, this must be recalculated \cite{tommy}. Figure \ref{fig:preprocessing/num_transfers} shows the distribution of the number of transfers per trip before and after patching. \selfnote{piechart might be better?}
...
\label{sec:partI}
\subsection{Features}
A first instance of the model will use features as used by Tu \cite{tu2016impact} for a fair model comparison. A second instance will contain all the variables described in Section \ref{sec:features}

\textbf{Tu features and representation}

\textbf{Proposed features and representation}
...
The 3 dimensional representation will prevent overfitting. 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/SVM_confusionMatrix"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/Random forest_confusionMatrix"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/AdaBoost of decision trees_confusionMatrix"}
  \caption{Random forest confusion matrix.}
  \label{fig:classification/adb_confusion}
\end{figure}

\subsubsection{3 dimensional representation}
