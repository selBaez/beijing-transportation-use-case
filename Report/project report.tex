...
In this section we look at studies within the last decade that are related to smart card transit data. First, we summarize the approach and the most relevant findings of each paper in order grasp a broad view of the Transportation domain. Secondly, we explore representations for spatiotemporal data and compare the way traveling behavior is usually represented in the Transportation domain, and other types of representations available in the Artificial Intelligence domain. Thirdly, we discuss techniques for pattern recognition through supervised and unsupervised learning. %Finally we explore some pioneer work in end-to-end learning. %TODO include if end to end learning
...
\begin{figure}[H]
  ...
  \caption{Number of trips distribution. 5,000 record sample.}
...
\begin{figure}[H]
  ...
  \caption{Distribution of start/end hours for trips. 5,000 records sample.}
  \label{fig:preprocessing/start_end_hour}
\end{figure}

From Figure \ref{fig:preprocessing/start_end_hour}, we note that our data follows the expected distribution for the domain, showing clear morning and evening peaks. Furthermore, we note that boarding and alighting patterns during the peaks hours are shifted by one hour. For example, the morning peak for boarding is at 7:00 am, while the alighting peaks at 8:00 am. Similarly, the evening peak happens between 6:00pm and 7:00pm for boarding, but between 19:00pm and 20:00pm for alighting. 
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{.7\textwidth}
  	\centering
  	\includegraphics[width=.85\linewidth]{"./images/Cummulative lines vocabulary size"}
  	\caption{Vocabulary containing subway lines and bus routes}
  \end{subfigure}
  \begin{subfigure}[b]{.7\textwidth}
  	\centering
  	\includegraphics[width=.85\linewidth]{"./images/Cummulative stops vocabulary size"}
...
\begin{figure}[H]
  ...
  \caption{Transfer number distribution before and after recalculation. 5,000 records sample.}
...
\begin{figure}[H]
  ...
  \caption{Travel distance vs travel time. 5,000 record sample.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Time and distance standardize distributions. 5,000 records sample.}
...
The mean travel time is \textasciitilde 31.9 minutes, and the mean travel distance is \textasciitilde 13.5 kilometers.  %TODO recalculate

%TRAVEL_TIME          1.916065e+06, TRAVEL_DISTANCE      1.353649e+04, TRANSFER_TIME_AVG    9.023386e+04, TRANSFER_TIME_SUM    1.017949e+05
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/Random forest"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/AdaBoost of decision trees"}
...
Interestingly, both classifiers have a similar behavior. Figures \ref{fig:classification/rf_confusion} and \ref{fig:classification/bag_confusion} show that both perform well in identifying commuters. Random forests achieves 80.0\% closely followed by Adaboost with 77.1\%. However, they both perform poorly when identifying Non-commuters, with 39.2\% and 42.8\% correspondingly. 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{"./images/Tu's features"}
...
The first block corresponds to the encoder, and the second block corresponds to the decoder modules. The dimensionality of the encoded features is $4 \times 5 \times 8 = 160$. This represents more than 110\% compression from the original dimensionality. 

We evaluate the autoencoder using the binary crossentropy loss between the original user cube and the reconstructed user cube. As an optimizer we use Adadelta. We train on 60\% of the data and validate on 40\% of the data for 200 iterations. The loss over the validation set is shown in Figure \ref{fig:clustering/loss}

%TODO explain it ran for 1000 iterations but it overfits
...
From Figure \ref{fig:clustering/tuning} we see that the average score peaks at 5 and 11 clusters. We select 5 as the optimal number of clusters. 

\subsection{Cluster analysis}
Figure \ref{fig:clustering/tsne} shows the samples, as visualized by TSNE, and their corresponding cluster label. 
...
\hline
On mode & None & None & None & None  & None \\ %or percentage by each of the three modes
